  ...
  return (
    <section className="relative pt-24 pb-16 overflow-hidden aurora-bg">
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} className="flex justify-center mb-6">
        ...
        >
          {title.split(" ").map((w, i) => (
            <span key={i} className={i % 2 ? "text-gradient-primary" : ""}>{w} </span>
          ))}
        </motion.h1>
        {subtitle && (
          <motion.p
